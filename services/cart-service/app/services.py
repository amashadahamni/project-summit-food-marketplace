...
def require_available_product(payload: CartItemRequest) -> None:
    ...
    if product.get("stock", 0) < payload.quantity:
        raise HTTPException(status_code=422, detail="Requested quantity exceeds available stock")


def add_or_update_item(database: Session, customer_id: str, payload: CartItemRequest) -> Cart:
    require_available_product(payload)
