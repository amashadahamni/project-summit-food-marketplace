...
def seed_demo_products(database: Session) -> None:
    ...
    existing_names = {product.name for product in ProductRepository(database).find_by_supplier("demo-supplier")}
    database.add_all([
...
def customer_products(database: Session, search: str | None, category: str | None, minimum_price: Decimal | None = None, maximum_price: Decimal | None = None, in_stock: bool = False) -> list[Product]:
    return ProductService(ProductRepository(database)).list_customer_products(search, category, minimum_price, maximum_price, in_stock)
